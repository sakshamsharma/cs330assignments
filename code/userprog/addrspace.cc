...
ProcessAddrSpace::ProcessAddrSpace(OpenFile *executable) {
  ...
  ASSERT(numPagesInVM + numPagesAllocated <=
         NumPhysPages); // check we're not trying
...
//----------------------------------------------------------------------
// ProcessAddrSpace::ProcessAddrSpace (ProcessAddrSpace*) is called by a forked
// thread.
...
ProcessAddrSpace::ProcessAddrSpace(ProcessAddrSpace *parentSpace) {
  ...
  for (i = 0; i < numPagesInVM; i++) {
    ...
    NachOSpageTable[i].readOnly =
        parentPageTable[i].readOnly; // if the code segment was entirely on
                                     // a separate page, we could set its
                                     // pages to be read-only
  }
  ...
  for (i = 0; i < size; i++) {
    machine->mainMemory[startAddrChild + i] =
        machine->mainMemory[startAddrParent + i];
  }
...
ProcessAddrSpace::~ProcessAddrSpace() { delete NachOSpageTable; }
...
void ProcessAddrSpace::InitUserCPURegisters() {
  int i;
...
unsigned ProcessAddrSpace::GetNumPages() { return numPagesInVM; }

TranslationEntry *ProcessAddrSpace::GetPageTable() { return NachOSpageTable; }
